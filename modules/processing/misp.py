...
class MISP(Processing):
	...
	# Patch: Read the blacklisted IOC's from the config file
        blacklist = self.options.get("ioc_blacklist", '').split(',')
        if ioc in blacklist:
            log.warning("MISP: Found blacklisted IOC: %s" % ioc)
            return
        try:
        ...
        for row in r.get("response", []):
            ...
            # Patch: Process only 'to_IDS' attributes?
	    only_ids = True
		...
		if attribute['value'] == ioc:
                    if only_ids and not attribute['to_ids']:
		    else:
